...
<table
  ...
  <thead>
    <tr
      ...
      <th class="px-4 py-3">
        {$_('human_resources.payslips.annual_earnings')}
      </th>
      <th class="px-4 py-3">
        {$_('human_resources.payslips.monthly_earnings')}
      </th>
      ...
      <th class="px-4 py-3">
        {$_('human_resources.payslips.sustainable_mobility_fee')}
      </th>
      <th class="px-4 py-3">{$_('human_resources.payslips.meal_tickets')}</th>
      <th class="px-4 py-3">
        {$_('human_resources.payslips.health_insurance')}
      </th>
      ...
      <th class="px-4 py-3">
        {$_('human_resources.payslips.exceptional_leaves')}
      </th>
    </tr>
  </thead>

  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
    {#each items as { firstName, lastName, contract, isExecutivePosition, joiningDate, annualEarnings, monthlyEarnings, workingTime, transportFee, sustainableMobilityFee, mealTickets, healthInsurance, paidLeaves, unpaidLeaves, sickLeaves, exceptionalLeaves }}
      <tr class="text-gray-700 dark:text-gray-400">
        <td class="px-4 py-3 text-sm">{firstName} {lastName}</td>
        <td class="px-4 py-3 text-sm">
          {contract}
          {isExecutivePosition ? $_('human_resources.payslips.executive_position') : ''}
        </td>
        ...
        <td class="px-4 py-3 text-sm">{format(sustainableMobilityFee)}</td>
        <td class="px-4 py-3 text-sm">{mealTickets}</td>
        <td class="px-4 py-3 text-sm">{$_(`common.${healthInsurance}`)}</td>
        <td class="px-4 py-3 text-sm">
          <Leave userLeaves={paidLeaves} />
        </td>
        <td class="px-4 py-3 text-sm">
          <Leave userLeaves={unpaidLeaves} />
        </td>
        <td class="px-4 py-3 text-sm">
          <Leave userLeaves={sickLeaves} />
        </td>
        <td class="px-4 py-3 text-sm">
          <Leave userLeaves={exceptionalLeaves} />
        </td>
      </tr>
    {/each}
  </tbody>
