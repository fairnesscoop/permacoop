<script>
  ...
  onMount(async () => {
    try {
      ({data} = await axios.get('pay_stubs'));
    } catch (e) {
...
<div class="col-md-12">
  ...
  <table class="table table-striped table-bordered table-hover">
    ...
    <tbody>
      {#each data as payStub (payStub.id)}
        <tr>
          <td>{format(new Date(payStub.date), 'MMMM yyyy', {locale: fr})}</td>
          <td>{payStub.user.firstName} {payStub.user.lastName}</td>
          <td>{payStub.file.originalName}</td>
          <td>
            {#if $user.id === payStub.user.id}
              <a class="btn btn-outline-secondary btn-sm" href="d">
                Télécharger ({filesize(payStub.file.size)})
              </a>
            {/if}
          </td>
        </tr>
