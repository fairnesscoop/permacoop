...
<form on:submit|preventDefault={submit} class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
  <Input label={'Nom du client'} bind:value={customer.name} />
  <Input label={'Adresse'} bind:value={address.street} />
  <Input label={'Code postal'} bind:value={address.zipCode} />
  <Input label={'Ville'} bind:value={address.city} />
