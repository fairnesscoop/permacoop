<script>
  ...
  const className = "block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input";
</script>
...
<div class={`block text-sm ${marginClass}`}>
  ...
  {:else if type === 'money'}
    <input type="number" min="0" placeholder="0,00" step="0.01" {id} {required} class={className} bind:value />
  {:else if type === 'date'}
