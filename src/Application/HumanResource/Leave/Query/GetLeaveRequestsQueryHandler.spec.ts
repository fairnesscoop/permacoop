...
describe('GetLeaveRequestsQueryHandler', () => {
  ...
  it('testGetLeaveRequests', async () => {
    ...
    when(leave1.getUserId()).thenReturn('eb9e1d9b-dce2-48a9-b64f-f0872f3157d2');
    ...
    when(leave2.getUserId()).thenReturn('eb9e1d9b-dce2-48a9-b64f-f0872f3157d2');
    ...
    when(leave3.getUserId()).thenReturn('abcffaa9-cdc5-40fa-a71c-0e93eadd61fd');
  ...
  it('testMenstrualLeaveIsMaskedForOtherUsers', async () => {
    const ownerId = 'eb9e1d9b-dce2-48a9-b64f-f0872f3157d2';
    const viewerId = 'abcffaa9-cdc5-40fa-a71c-0e93eadd61fd';
    ...
    const expectedResult = new Pagination<LeaveRequestView>(
      [
        new LeaveRequestView(
          ...
          true,
          null,
          new UserSummaryView(ownerId, 'Jane', 'DOE')
        )
      ],
      1
    );

    const leaveOwner = mock(User);
    when(leaveOwner.getId()).thenReturn(ownerId);
    when(leaveOwner.getFirstName()).thenReturn('Jane');
    when(leaveOwner.getLastName()).thenReturn('DOE');
    ...
    when(menstrualLeave.getStartDate()).thenReturn('2020-05-08');
    when(menstrualLeave.isStartsAllDay()).thenReturn(true);
    when(menstrualLeave.getEndDate()).thenReturn('2020-05-15');
    when(menstrualLeave.isEndsAllDay()).thenReturn(true);
    when(menstrualLeave.getUser()).thenReturn(instance(leaveOwner));
    ...
    const queryHandler = new GetLeaveRequestsQueryHandler(
      instance(leaveRequestRepository),
      instance(userRepository),
      instance(dateUtils),
      instance(canLeaveRequestBeCancelled)
