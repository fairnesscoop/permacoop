...
describe('GetPaySlipsQueryHandler', () => {
  it('testGetPaySlips', async () => {
    ...
    when(paySlipRepository.findAll()).thenResolve([
      instance(paySlip1),
      instance(paySlip2)
    ]);
    ...
    expect(await queryHandler.execute(new GetPaySlipsQuery())).toMatchObject(
      expectedResult
    );

    verify(paySlipRepository.findAll()).once();
  });

  it('testGetEmptyPaySlips', async () => {
    const paySlipRepository = mock(PaySlipRepository);
    when(paySlipRepository.findAll()).thenResolve([]);
